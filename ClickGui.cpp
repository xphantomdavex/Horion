...
static const MC_Color selectedModuleColor = MC_Color(184, 255, 242);
static const MC_Color moduleColor = MC_Color(170, 248, 105);
...
void ClickGui::renderCategory(Category category) {
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		for (auto& mod : moduleList) {
			...
			// Text
			if (allowRender)
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? MC_Color(94, 68, 255) : MC_Color(147, 68, 255), textSize);
